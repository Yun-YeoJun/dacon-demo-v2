...
export function AnalysisResult({ onNavigate, messageText, analysisResponse, apiError }: AnalysisResultProps) {
  ...
  return (
    <div className="h-full flex flex-col pt-8">
      ...
      <div className="mx-4 my-6 bg-gradient-to-br from-red-400 to-pink-500 rounded-3xl p-8 text-white text-center shadow-lg">
        <div className="text-6xl mb-4">⚠️</div>
        <h2 className="text-2xl font-bold">위험한 메시지입니다</h2>
        {analysisResponse?.result?.score != null && (
          <div className="mt-2 text-sm opacity-90">위험 점수: {analysisResponse.result.score}</div>
        )}
      </div>
      ...
      <div className="mx-4 mb-6">
        ...
        {reasons.length === 0 ? (
          ...
          <div className="space-y-3">
            {reasons.map((r, idx) => (
              <div key={idx} className="flex items-start gap-3 bg-red-50 rounded-xl p-4">
